        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers=HEADERS) as response:
                ...
                if pos == count:
                    ...
                    aa = jj.find("div",class_="flex flex-col")
                    zz = aa.find("div",class_="flex flex-col gap-2 contentP")
                    yy = zz.find("div",class_="grid grid-cols-1 2md:grid 2md:grid-cols-2 gap-2 text-sm")
...
if __name__ == "__main__":
    ...
    loop.run_until_complete(main("Puck","1"))